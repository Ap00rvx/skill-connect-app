...
class TodoService {
  ...
  Future<List<Todo>> getToDoList(DateTime date) async {
    ...
    try {
      return list.map((taskJson) {
        // Log each JSON string before decoding
  ...
  Future<List<Todo>> filterCompletedTodos(
    DateTime date,
  ) async {
    final tasks = await getToDoList(date);
    return tasks.where((element) => element.isCompleted).toList();
  }

  Future<List<Todo>> filterIncompletedTodos(DateTime date) async {
    final tasks = await getToDoList(date);
    return tasks.where((element) => (!element.isCompleted)).toList();
  }
}
